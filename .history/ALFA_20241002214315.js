...
async function convertAndCompressImage(filePath) {
    try {
        console.log('Procesando imagen:', filePath);  // Verificar qué imagen se está procesando
        const metadata = await sharp(filePath).metadata();
        let outputFilePath = filePath.replace(path.extname(filePath), '-compressed.jpg');  // Cambiar el nombre del archivo de salida

        // Si la imagen es HEIC, convertirla primero
        if (metadata.format === 'heic') {
            console.log('Detectado formato HEIC. Usando heic-convert para convertir a JPG...');
            const inputBuffer = await promisify(fs.readFile)(filePath);
            const outputBuffer = await convert({
                buffer: inputBuffer, // el archivo HEIC como buffer
                format: 'JPEG', // formato de salida
                quality: 0.8,   // calidad
            });
            const heicConvertedPath = filePath.replace('.heic', '.jpg');
            fs.writeFileSync(heicConvertedPath, outputBuffer);
            fs.unlinkSync(filePath);  // Eliminar archivo original HEIC
            filePath = heicConvertedPath;  // Actualizar el nuevo path para seguir la compresión
        }

        // Para todos los archivos, incluyendo los HEIC ya convertidos, usar sharp para comprimir
