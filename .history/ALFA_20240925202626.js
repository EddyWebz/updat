...
router.post('/register', [verifyJWT, upload.array('image', 4)], async (req, res) => {
    ...
    const user_name = req.userName;  // Obtener el nombre del usuario del token JWT
      ...
      // Agregar log para verificar si se están obteniendo los valores correctamente
      console.log('Usuario ID:', user_id);  // Verificar el ID del usuario
      console.log('Usuario Name:', user_name);  // Verificar el nombre del usuario
    ...
    try {
        ...
        const eventDetails = `Registro de vehículo con placa: ${plate}, propietario: ${owner}, habitación: ${habitacion}`;
