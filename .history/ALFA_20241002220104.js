...
async function convertAndCompressImage(filePath) {
    let outputFilePath = filePath.replace(path.extname(filePath), '-compressed.jpg');
    try {
        console.log('Procesando imagen:', filePath);

        // Si el formato es HEIC, usar `heic-convert` para convertir a JPEG
        if (filePath.endsWith('.heic')) {
            console.log('Convirtiendo archivo HEIC a JPG...');
            const inputBuffer = fs.readFileSync(filePath); // Leer el archivo HEIC
            const outputBuffer = await convert({
                buffer: inputBuffer,  // El archivo HEIC en buffer
                format: 'JPEG',       // Convertir a JPEG
                quality: 1            // Calidad máxima
            });
            fs.writeFileSync(outputFilePath, outputBuffer); // Escribir el archivo convertido
            fs.unlinkSync(filePath);  // Eliminar el archivo original HEIC
        } else {
            console.log('El archivo no es HEIC, no necesita conversión.');
        }
