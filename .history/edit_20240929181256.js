...
router.put('/vehicle/:id', [verifyJWT, upload.array('image', 4)], async (req, res) => {
    ...
    const { brand, model, clave, plate, color, owner, stayNights, habitacion, garage, observations, existingImages, imagesToReplace } = req.body;

    // Inicializar variable de imágenes
    let imagesJson = [];

    // Establecer el límite máximo de imágenes permitidas (4 en tu caso)
    const maxImages = 4;
    ...
    try {
        ...
        // Parsear las imágenes existentes si están disponibles
        if (existingImages) {
        ...
        if (req.files && req.files.length > 0) {
            ...
            // Si se seleccionaron imágenes para reemplazar
            if (imagesToReplace && imagesToReplace.length > 0) {
                const imagesToReplaceArray = JSON.parse(imagesToReplace);

                // Remover las imágenes seleccionadas para reemplazar de las existentes
                imagesJson = imagesJson.filter(image => !imagesToReplaceArray.includes(image));
            }

            // Verificar si el número total de imágenes (existentes + nuevas) excede el límite permitido
            if (imagesJson.length + newImages.length > maxImages) {
                return res.status(400).json({
                    success: false,
                    message: `No puedes subir más de ${maxImages} imágenes en total. Ya tienes ${imagesJson.length} imágenes y estás intentando subir ${newImages.length}. Solo puedes subir ${maxImages - imagesJson.length} imágenes más.`
                });
            }

            // Añadir las nuevas imágenes a las existentes
            imagesJson = [...imagesJson, ...newImages];
        }
        ...
        if (currentVehicle.observations !== observations) updateDetails += `Observaciones anteriores: ${currentVehicle.observations} \n Nuevas observaciones: ${observations}.\n`;
        ...
        const values = [brand, model, clave, plate, color, owner, stayNights, habitacion, garage, observations, JSON.stringify(imagesJson), vehicleId];
