...
// Función para detectar y convertir imágenes
...
router.post('/register', [verifyJWT, upload.array('image', 4)], async (req, res) => {
    ...
    for (let file of req.files) {
        try {
            const convertedPath = await convertImage(file.path);  // Convertir la imagen si es necesario
            images.push(`/uploads/${path.basename(convertedPath)}`);
        } catch (error) {
