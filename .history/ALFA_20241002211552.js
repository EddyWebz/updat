...
// Función para convertir imágenes HEIC a JPEG
async function heicToJpeg(filePath) {
    try {
        const inputBuffer = await promisify(fs.readFile)(filePath);
        const outputBuffer = await convert({
            buffer: inputBuffer,
            format: 'JPEG'  // Convertir a formato JPEG
        });

        const outputFilePath = filePath.replace(path.extname(filePath), '-converted.jpg');
        await promisify(fs.writeFile)(outputFilePath, outputBuffer);
        fs.unlinkSync(filePath);  // Eliminar el archivo HEIC original

        return outputFilePath;
    } catch (error) {
        console.error('Error al convertir el archivo HEIC:', error);
        throw new Error('Error al convertir el archivo HEIC: ' + error.message);
    }
}

// Función para convertir y comprimir imágenes
async function convertAndCompressImage(filePath) {
    try {
        ...
        let outputFilePath = filePath.replace(path.extname(filePath), '-compressed.jpg');  // Cambia el nombre del archivo de salida

        // Si la imagen es HEIC, convertirla primero
        if (metadata.format === 'heic') {
            filePath = await heicToJpeg(filePath);
            outputFilePath = filePath.replace(path.extname(filePath), '-compressed.jpg');
        }

        // Comprimir la imagen si ya es JPEG o PNG
        await sharp(filePath)
            .jpeg({ quality: 80 })  // Comprimir la imagen al 80% de calidad
            .toFile(outputFilePath);

        fs.unlinkSync(filePath);  // Eliminar el archivo original después de la conversión y compresión
