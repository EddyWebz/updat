...
// Ruta en el backend para devolver el user_id
app.get('/api/get-user-info', (req, res) => {
    const token = req.cookies.token; // Suponiendo que el token está en las cookies
    ...
    try {
        const decoded = jwt.verify(token, secretKey); // Decodificar el token usando la clave secreta del servidor
        return res.json({ user_id: decoded.userId }); // Retornar el user_id decodificado del token
    } catch (error) {
