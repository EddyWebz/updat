...
router.put('/vehicle/:id', [verifyJWT, upload.array('image', 4)], async (req, res) => {
    ...
    try {
        ...
        // Si se suben nuevas imágenes, las procesamos y combinamos con las imágenes existentes no reemplazadas
        if (req.files && req.files.length > 0) {
            ...
            // Combinamos las imágenes existentes que no se reemplazan con las nuevas imágenes
            if (existingImages) {
                const existingImagesArray = JSON.parse(existingImages);  // Parsear las imágenes existentes
                imagesJson = [...existingImagesArray, ...newImages];  // Combinar las imágenes que se mantienen con las nuevas
            } else {
                imagesJson = newImages;  // Solo nuevas imágenes si no hay existentes
            }
        } else if (existingImages) {
            imagesJson = JSON.parse(existingImages);  // Mantener solo las imágenes existentes si no hay nuevas
        }
        ...
        const values = [brand, model, clave, plate, color, owner, stayNights, habitacion, garage, observations, JSON.stringify(imagesJson), vehicleId];  // Asegurarse de que imagesJson se convierte en una cadena JSON
