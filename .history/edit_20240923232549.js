...
router.get('/get-user-role', verifyJWT, async (req, res) => {
    try {
        console.log('Verificando el rol del usuario con ID:', req.userId);  // Log para verificar el userId
        const [user] = await connection.query('SELECT role FROM usuarios WHERE id = ?', [req.userId]);
        if (user.length > 0) {
            console.log('Rol del usuario obtenido:', user[0].role);  // Log para mostrar el rol obtenido
