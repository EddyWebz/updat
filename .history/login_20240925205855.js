...
function generateToken(userId, role, nombre = null, name = null) {
    // Decidir qué valor de nombre usar
    const finalName = nombre || name;  // Usa `nombre` si existe, de lo contrario `name`
    
    // Incluir el nombre en el payload
    const payload = { userId, role, name: finalName };  
...
router.post('/login', async (req, res) => {
    ...
    if (isMatch) {
        // Generar JWT con el nombre y rol del usuario principal
      // Generar JWT con el `nombre` del usuario principal
      const token = generateToken(mainUser.id, mainUser.role, mainUser.nombre);  // Utiliza `mainUser.nombre`
        res.cookie('token', token, {
            httpOnly: true,
            secure: process.env.NODE_ENV === 'production',
            maxAge: 24 * 60 * 60 * 1000  // 24 horas
