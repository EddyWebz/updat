    ...
    try {
        ...
        if (results.length > 0) {
            ...
            if (isMatch) {
                ...
                if (role === 'admin') {
                    res.status(200).json({ success: true, message: 'Inicio de sesión exitoso', redirect: '/privado/admin.html' });
                } else {
                    res.status(200).json({ success: true, redirect: '/privado/cuerpo.html' });  // Ruta para usuarios normales
                }
            } else {
