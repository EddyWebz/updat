...
function openEditForm(vehicle) {
    ...
    imagePreviewContainer.innerHTML = '';  // Limpiar la vista previa existente

    // Asegurarnos de que se puedan mostrar hasta 4 imágenes
    if (vehicle.images && vehicle.images.length > 0) {
        vehicle.images.slice(0, 4).forEach(imageUrl => {
            const imgElement = document.createElement('img');
            imgElement.src = imageUrl;  // Mostrar la imagen actual
            imgElement.alt = 'Imagen del vehículo';
            imgElement.className = 'image-preview';  // Clase para el estilo de las imágenes
...
document.getElementById('vehicleEditForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            refreshSearch();  // Llamar a la función que refresca la búsqueda
        } else {
...
// Función para refrescar la búsqueda automáticamente después de la actualización
function refreshSearch() {
    const query = document.getElementById('searchInput').value.trim();
    const { startDate, endDate } = getSelectedDates();  // Usar la función que maneja las fechas

    // Realizar la búsqueda con la consulta actual
        ...
        .then(vehicles => {
            ...
            if (vehicles.length === 0) {
                ...
                vehicles.forEach(vehicle => {
                    ...
                    // Manejar imágenes y botón de edición
                    handleVehicleImages(vehicle, card);
