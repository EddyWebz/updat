...
// Función para obtener el user_id desde el backend
async function getUserId() {
    try {
        ...
        if (data && data.user_id) {
            return data.user_id;  // Aquí obtienes el user_id de la sesión actual
        } else {
...
// Función para guardar los datos del formulario usando el user_id y un timestamp
async function saveFormData() {
    const userId = await getUserId();  // Solicitar el user_id
    console.log('userId obtenido:', userId);

    if (!userId) {
        console.error('No se encontró user_id. No se puede guardar el formulario.');
        return;
    }
    ...
    const formData = {
        ...
        timestamp: new Date().getTime()  // Guardar el timestamp del momento actual
    };

    // Guardar en el Local Storage con el ID del usuario
    localStorage.setItem(`vehicleFormData_${userId}`, JSON.stringify(formData));
    console.log(`Datos guardados en Local Storage para el user_id ${userId}:`, formData);
}

// Función para cargar los datos del formulario desde el Local Storage y verificar si han expirado
async function loadFormData() {
    console.log(`Obteniendo datos para el userId: ${userId}`);

    const userId = await getUserId(); // Obtener el user_id mediante la petición al backend
    console.log('Cargando los datos del usuario: ', userId);

    if (!userId) {
        console.error('No se pudo cargar el formulario porque no se encontró el user_id.');
        return;
    }

    const savedData = localStorage.getItem(`vehicleFormData_${userId}`); // Obtener datos asociados a este user_id
    if (savedData) {
        const formData = JSON.parse(savedData);
        const currentTime = Date.now(); // Obtener el timestamp actual
        // Verificar si los datos han expirado (más de 40 minutos)
        if (currentTime - formData.timestamp > 40 * 60 * 1000) {
            console.log("Los datos han expirado, eliminando...");
            localStorage.removeItem(`vehicleFormData_${userId}`); // Eliminar los datos si han expirado
            return;
        }

        // Si los datos no han expirado, cargar en el formulario
        console.log(`Datos cargados desde Local Storage para el user_id ${userId}:`, formData);
...
formElements.forEach(element => {
    element.addEventListener('input', saveFormData); // Guardar al cambiar cualquier campo
    element.addEventListener('change', saveFormData); // Guardar al cambiar el valor de un select o textarea
});

// Cargar los datos cuando la página se cargue
document.addEventListener('DOMContentLoaded', () => {
    loadFormData();  // Cargar los datos guardados del Local Storage
});

// ---- LIMPIAR EL LOCAL STORAGE CUANDO SE ENVÍA EL FORMULARIO ----
document.getElementById('vehicleForm').addEventListener('submit', async () => {
    const userId = await getUserId(); // Obtener el user_id mediante la petición al backend
    if (userId) {
        localStorage.removeItem(`vehicleFormData_${userId}`);  // Eliminar los datos guardados del usuario actual
        console.log(`Datos eliminados del Local Storage para el user_id ${userId}`);
    }
});
// Ejecutar cuando el DOM esté completamente cargado
document.addEventListener('DOMContentLoaded', async () => {
    console.log('Página cargada, intentando cargar los datos...');
    await loadFormData();  // Invocar la función que carga los datos del formulario
});
...
const translations = {
    ...
    function formatDate(dateString) {
       ...
       botonhistor.addEventListener('click', async () => {
        const isAuthenticated = await checkAuthentication();
        if (!isAuthenticated) return;  // Redirige si no está autenticado
