    ...
    if (celdaPlaca) {  // Asegúrate de que el elemento existe
        if (!celdaPlaca.querySelector('input')) {
            ...
            guardarBtn.addEventListener('click', function () {
                ...
                guardarComentarioEnBaseDatos(id, input.value);  // Enviamos solo el comentario
                input.remove();
                guardarBtn.remove();
            });
...
async function generarReporteAutomatico() {
    ...
    try {
        ...
        data.P1.forEach((registro) => {
            ...
            mostrarComentario(registro.id, registro.comentario);  // Asegúrate de recibir el comentario en la data
        });
