        ...
        eventoElement.classList.add('evento');
           ...
           if (evento.accion === 'Registro') {
            ...
            eventoElement.innerHTML = `
                <p><strong>${evento.usuario}</strong> registró un vehículo con placa <strong>${evento.vehiculo_placa}</strong>, propietario <strong>${evento.propietario}</strong>, habitación <strong>${evento.habitacion}</strong></p>
                <p class="fecha">${new Date(evento.fecha).toLocaleString()}</p>
            `;
        } else {
