...
function openEditForm(vehicle) {
    ...
    function openEditForm(vehicle) {
        ...
        // Pre-llenar los campos del vehículo
        document.getElementById('vehicleId').value = vehicle.id;
        // (Rellenar otros campos como ya tienes implementado...)
    
        // Mostrar las imágenes actuales con opción de seleccionar para reemplazar
        ...
        if (vehicle.images && vehicle.images.length > 0) {
            vehicle.images.slice(0, 4).forEach((imageUrl, index) => {
                ...
                imgElement.style.transition = 'all 0.3s ease';  // Añadir una transición suave para el efecto visual
                div.appendChild(imgElement);
    
                // Checkbox para seleccionar la imagen a reemplazar
                const checkbox = document.createElement('input');
                checkbox.type = 'checkbox';
                checkbox.name = 'replaceImage';
                checkbox.value = index;  // El índice de la imagen
                checkbox.className = 'replace-checkbox';
                div.appendChild(checkbox);
    
                // Evento para cambiar el estilo de la imagen cuando se seleccione el checkbox
                checkbox.addEventListener('change', (e) => {
                    if (e.target.checked) {
                        imgElement.style.border = '3px solid red';  // Añadir un borde rojo alrededor de la imagen seleccionada
                        imgElement.style.opacity = '0.7';  // Reducir la opacidad para un efecto visual
                    } else {
                        imgElement.style.border = 'none';  // Quitar el borde si se desmarca
                        imgElement.style.opacity = '1';  // Restaurar la opacidad
                    }
                });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('vehicleEditForm').addEventListener('submit', async (e) => {
        ...
        // Verificar si hay imágenes seleccionadas para reemplazar
        const selectedIndexes = [];
        const checkboxes = document.querySelectorAll('input[name="replaceImage"]:checked');
        checkboxes.forEach(checkbox => {
            selectedIndexes.push(parseInt(checkbox.value));  // Guardar los índices seleccionados
        });
    
        // Añadir los índices seleccionados al formData
        if (selectedIndexes.length > 0) {
            formData.append('replaceIndexes', JSON.stringify(selectedIndexes));  // Enviar los índices seleccionados
        } else {
            formData.append('replaceAll', 'true');  // Si no hay selecciones, indicar que se reemplazan todas
        }
