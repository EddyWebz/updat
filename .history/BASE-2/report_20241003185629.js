...
async function guardarComentarioEnBaseDatos(id, comentario, expiracion) {
    console.log('Enviando comentario a la base de datos...');  // Confirmar que se llama a la función
    console.log('ID:', id, 'Comentario:', comentario, 'Expiración:', expiracion);  // Mostrar los datos que se envían
    try {
        const response = await fetch('/api/comentario', {
            ...
            body: JSON.stringify({ id, comentario, expiracion })  // Enviamos el comentario y la expiración
        });
    ...
    if (celdaPlaca) {  // Asegúrate de que el elemento existe
        if (!celdaPlaca.querySelector('input')) {
            ...
            guardarBtn.addEventListener('click', function () {
                console.log('Botón Guardar clickeado, guardando comentario...');  // Confirmación de que se hizo clic en guardar
                // Calcula la fecha de expiración (ejemplo 2 horas)
                const expiracion = new Date();
                expiracion.setHours(expiracion.getHours() + 2);  // Expiración en 2 horas
                // Convertir la fecha al formato 'YYYY-MM-DD HH:MM:SS' para MySQL
const expiracionFormatoMySQL = expiracion.toISOString().slice(0, 19).replace('T', ' ');
console.log(expiracionFormatoMySQL);  // '2024-10-04 01:50:55'
