...
function openCommentInput(id, placa) {
    ...
    if (celdaPlaca) {  // Asegúrate de que el elemento existe
        if (!celdaPlaca.querySelector('input')) {
            ...
            input.value = comentarioExistente;  // Esto permite editar el comentario existente
 ...
 guardarBtn.addEventListener('click', async function () {
     ...
     try {
         await guardarComentarioEnBaseDatos(id, input.value);  // Guardar el nuevo comentario (o el editado)
         input.remove();  // Eliminar el input después de guardar
         guardarBtn.remove();  // Eliminar el botón después de guardar
         console.log('Comentario guardado y elementos eliminados');
     } catch (error) {
