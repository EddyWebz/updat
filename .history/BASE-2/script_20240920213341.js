...
function loadFormData() {
    const userId = getCookie('user_id'); // Obtener el user_id desde las cookies

    if (userId) {
        const savedData = localStorage.getItem(`vehicleFormData_${userId}`); // Obtener datos asociados a este user_id
        if (savedData) {
            const formData = JSON.parse(savedData);
            console.log(`Datos cargados desde Local Storage para el user_id ${userId}:`, formData); // Mostrar los datos cargados
