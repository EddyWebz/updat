...
// Función para mostrar el comentario en el DOM
...
guardarBtn.addEventListener('click', async function () {
    try {
        ...
        // Asegurarse de eliminar el input y los botones tras guardar
...
eliminarBtn.addEventListener('click', async function () {
    try {
        ...
        mostrarComentario(id, '');  // Reflejar el cambio inmediatamente en el DOM
        
        // Asegurar eliminación visual con animación de desvanecimiento
        const comentarioDiv = celdaPlaca.querySelector('.comentario');
        if (comentarioDiv) {
            comentarioDiv.classList.add('fade-out');  // Aplicar clase para animación de desvanecimiento
            setTimeout(() => {
                comentarioDiv.remove();  // Eliminar el comentario del DOM
            }, 300);  // Tiempo para la animación
        }

        // Eliminar también el input y los botones
        input.remove();  // Eliminar el input
        guardarBtn.remove();  // Eliminar el botón de guardar
        eliminarBtn.remove();  // Eliminar el botón de eliminar
