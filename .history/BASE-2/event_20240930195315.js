document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        // Construir la URL por defecto con las últimas 24 horas
...
document.getElementById('applyFilter').addEventListener('click', async function() {
    try {
        ...
        // Verificación de los valores de las fechas
        console.log(`Fecha de inicio seleccionada: ${startDate}`);
        console.log(`Fecha de fin seleccionada: ${endDate}`);

        // Si no hay fechas seleccionadas, mostrar un mensaje de error
        if (!startDate || !endDate) {
            console.error('Fechas no seleccionadas correctamente');
            return;
        }

        // Construir la URL con los parámetros de fecha seleccionados
        let url = `/api/eventos?startDate=${startDate}&endDate=${endDate}`;

        // Log para ver la URL que se está enviando
        console.log(`Solicitando eventos con URL: ${url}`);

        // Enviar solicitud GET con las fechas seleccionadas
        const response = await fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
