...
function openCommentInput(id, placa) {
    console.log('Abriendo input para comentario de la placa:', placa);  // Confirmación de que se ha hecho clic
    ...
    if (celdaPlaca) {  // Asegúrate de que el elemento existe
        if (!celdaPlaca.querySelector('input')) {
            ...
            // Cargar el comentario existente (si ya existe)
            const comentarioExistente = celdaPlaca.querySelector('.comentario') ? celdaPlaca.querySelector('.comentario').innerText : '';
            input.value = comentarioExistente;  // Permitir editar el comentario existente
            ...
            guardarBtn.addEventListener('click', async function () {
                console.log('Botón Guardar clickeado, guardando comentario...');
                try {
                    await guardarComentarioEnBaseDatos(id, input.value || '');  // Guardar el nuevo comentario (permitir vacío para eliminar)
                    mostrarComentario(id, input.value);  // Reflejar el cambio inmediatamente en el DOM
                    input.remove();  // Eliminar el input después de guardar
                    guardarBtn.remove();
                    eliminarBtn.remove();
                    console.log('Comentario guardado (o eliminado si estaba vacío)');
                } catch (error) {
                    console.error('Error al guardar el comentario:', error);
                }
            });

            // Eliminar el comentario (borrar texto y guardar automáticamente)
            eliminarBtn.addEventListener('click', async function () {
                console.log('Botón Eliminar clickeado, eliminando comentario...');
                try {
                    input.value = '';  // Borrar el texto del input
                    await guardarComentarioEnBaseDatos(id, '');  // Guardar cadena vacía automáticamente
                    mostrarComentario(id, '');  // Actualizar el DOM inmediatamente
                    input.remove();
                    guardarBtn.remove();
                    eliminarBtn.remove();
                    console.log('Comentario eliminado y guardado automáticamente');
                } catch (error) {
...
// Función para mostrar el comentario a continuación de la habitación
function mostrarComentario(id, comentario) {
    const celdaPlaca = document.querySelector(`td[data-id="${id}"]`);

    if (celdaPlaca) {
        // Obtener el contenido actual de la celda (por ejemplo, el número de habitación)
        const habitacion = celdaPlaca.getAttribute('data-habitacion') || celdaPlaca.innerText;
        
        // Si hay un comentario, lo mostramos junto a la habitación
        const textoFinal = comentario ? `${habitacion} - ${comentario}` : habitacion;

        // Actualizar el contenido de la celda con la habitación y el comentario en la misma línea
        celdaPlaca.innerText = textoFinal;
    } else {
        console.error(`La celda con el id ${id} no se encontró en el DOM`);
    }
}
...
data.P1.forEach((registro) => {
    ...
    // Incluir el comentario junto a la habitación si existe
