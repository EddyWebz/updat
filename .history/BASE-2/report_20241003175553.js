...
function openCommentInput(id, placa) {
    ...
    if (!celdaPlaca.querySelector('input')) {
        ...
        guardarBtn.addEventListener('click', function () {
            // Aquí calculas la fecha de expiración, por ejemplo 2 horas desde el momento actual
            const expiracion = new Date();
            expiracion.setHours(expiracion.getHours() + 2);  // Ejemplo: Expira en 2 horas
...
// Función para guardar el comentario y la expiración en la base de datos
async function guardarComentarioEnBaseDatos(id, comentario, expiracion) {
    try {
        const response = await fetch('/api/vehiculos/comentario', {
            ...
            body: JSON.stringify({ id, comentario, expiracion })  // Enviamos el comentario y la fecha de expiración
        });
...
function mostrarComentario(id, comentario) {
    const celdaPlaca = document.querySelector(`td[data-id="${id}"]`);
    const item = localStorage.getItem(`comentario_${id}`);

    if (item) {
        const datos = JSON.parse(item);

        const ahora = new Date().getTime();
        const expiracion = new Date(datos.expiracion).getTime();

        // Solo mostrar el comentario si no ha expirado
        if (ahora < expiracion) {
            ...
            // Eliminar el comentario si ha expirado
            if (comentarioExistente) {
