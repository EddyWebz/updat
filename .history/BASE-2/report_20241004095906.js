...
function openCommentInput(celdaPlaca, id) {
    console.log('Abriendo input para comentario de la habitación:', id);
    ...
    if (celdaPlaca) {
        if (!celdaPlaca.querySelector('input')) {
            ...
            guardarBtn.addEventListener('click', async function () {
                ...
                try {
                    await guardarComentarioEnBaseDatos(id, input.value || '');
                    mostrarComentario(celdaPlaca, input.value);
                    input.remove();
                    guardarBtn.remove();
                    eliminarBtn.remove();
            ...
            eliminarBtn.addEventListener('click', async function () {
                ...
                try {
                    await guardarComentarioEnBaseDatos(id, '');
                    mostrarComentario(celdaPlaca, '');
                    input.remove();
                    guardarBtn.remove();
                    eliminarBtn.remove();
...
//MOSTRAR COMENTARIO
function mostrarComentario(celdaPlaca, comentario) {
    if (celdaPlaca) {
...
async function generarReporteAutomatico() {
    ...
    try {
        ...
        data.P1.forEach((registro) => {
            ...
            tr.innerHTML = `<td class="celda-habitacion" data-id="${registro.id}">${habitacionConComentario}</td>
