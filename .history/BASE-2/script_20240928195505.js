document.addEventListener('DOMContentLoaded', async () => {
    ...
    const captureButton = document.getElementById('captureButton'); // Botón para capturar imágenes
    const previewContainer = document.getElementById('previewContainer'); // Contenedor de previsualizaciones
    const capturedImagesInput = document.getElementById('capturedImages'); // Campo oculto para imágenes capturadas
    ...
    imageInput.addEventListener('change', async (e) => {
        ...
        if ((imagesCaptured.length + files.length) > maxImages) {
        ...
        previewContainer.innerHTML = ''; // Limpiar el contenedor de previsualizaciones
    ...
    imageInput.addEventListener('change', () => {
        if ((imagesCaptured.length + imageInput.files.length) > maxImages) {
            alert('Solo puedes subir un máximo de 4 imágenes en total.');
            imageInput.value = '';  // Vaciar la selección
        }
    });
    ...
    captureButton.addEventListener('click', () => {
        if (imagesCaptured.length >= maxImages) {
            alert(`Has alcanzado el límite de ${maxImages} imágenes.`);
            return;
        }
        startCamera();
    });
    ...
    function startCamera() {
        navigator.mediaDevices.getUserMedia({ video: true })
            .then(stream => {
                // Crear elementos para la visualización de la cámara y botones
                ...
                captureImageButton.textContent = 'Capturar';
                document.body.appendChild(captureImageButton);

                const closeCameraButton = document.createElement('button');
                closeCameraButton.textContent = 'Cerrar Cámara';
                document.body.appendChild(closeCameraButton);

                captureImageButton.addEventListener('click', () => {
                    captureImage(video);
                    if (imagesCaptured.length >= maxImages) {
                        alert('Has alcanzado el límite de imágenes.');
                        stopCamera(video, stream, captureImageButton, closeCameraButton);
                    }
                });
    ...
    function captureImage(video) {
        ...
        capturedImagesInput.value = JSON.stringify(imagesCaptured.map(b => b.name));
    }

    function stopCamera(video, stream, captureImageButton, closeCameraButton) {
        // Detener el stream
    ...
    vehicleForm.addEventListener('submit', async (e) => {
        ...
        // Agregar las imágenes capturadas al FormData
        for (let i = 0; i < imagesCaptured.length; i++) {
            const blob = imagesCaptured[i];
            const compressedBlob = await compressBlob(blob);
            formData.append('image', compressedBlob, `capturedImage${i}.jpg`);
        }

        // Las imágenes seleccionadas ya están comprimidas en el evento 'change' del input
        const selectedFiles = imageInput.files;
        for (let file of selectedFiles) {
            formData.append('image', file, file.name);
        }

        // Validar el límite de imágenes
        if ((imagesCaptured.length + selectedFiles.length) > maxImages) {
            alert('Solo puedes subir un máximo de 4 imágenes en total.');
    ...
    // Función para comprimir imágenes desde Blob
    ...
    // (Mantener el resto de tu código sin modificaciones)
