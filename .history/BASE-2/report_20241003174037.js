...
function openCommentInput(id, placa) {
    ...
    if (!celdaPlaca.querySelector('input')) {
        ...
        if (item) {
            ...
            input.value = datos.comentario;  // Mostrar el comentario si existe
        }
        ...
        guardarBtn.addEventListener('click', function () {
            guardarComentario(id, input.value);  // Guardar comentario por ID
...
function guardarComentario(id, comentario) {
    // Guardar incluso si el comentario está vacío
    const ahora = new Date().getTime();
    const expiracion = ahora + 2 * 60 * 60 * 1000; // 2 horas

    const comentarioConExpiracion = {
        comentario: comentario,  // Puede ser vacío
        expiracion: expiracion
    };

    localStorage.setItem(`comentario_${id}`, JSON.stringify(comentarioConExpiracion));
