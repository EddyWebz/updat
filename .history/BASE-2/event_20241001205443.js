document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        if (!fechaInicioInput || !fechaFinInput) {
            const now = new Date();
            fechaFin = now.toISOString().split('T')[0] + ' 23:59:59'; // Hoy a las 23:59
            fechaInicio = new Date(now.getTime() - 24 * 60 * 60 * 1000).toISOString().split('T')[0] + ' 00:00:00'; // Ayer a las 00:00
            ...
            fechaInicio = `${fechaInicioInput} 00:00:00`; // El día comienza a las 00:00
            fechaFin = `${fechaFinInput} 23:59:59`; // El día termina a las 23:59
        }

        // Agregamos un console.log para verificar las fechas que se envían
        console.log(`Fechas enviadas al backend: Inicio = ${fechaInicio}, Fin = ${fechaFin}`);

        // Construir la URL con los parámetros de fecha
        let url = `/api/eventos?startDate=${encodeURIComponent(fechaInicio)}&endDate=${encodeURIComponent(fechaFin)}`;
