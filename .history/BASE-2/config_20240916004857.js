...
function openEditForm(vehicle) {
  ...
  // Obtener la fecha en formato local y mantener la hora local exacta
  const vehicleDatetime = new Date(vehicle.datetime);
  const timezoneOffset = vehicleDatetime.getTimezoneOffset() * 60000;  // Obtener el desfase de la zona horaria en milisegundos
  const localDatetime = new Date(vehicleDatetime.getTime() - timezoneOffset).toISOString().slice(0, 16);
     document.getElementById('editDatetime').value = formattedDatetime;// Tomar la fecha desde la base de datos
    document.getElementById('editDatetime').value = formattedDatetime;
    document.getElementById('editDatetime').value = new Date(vehicle.datetime).toISOString().slice(0, 16);
