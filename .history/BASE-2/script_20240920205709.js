...
async function saveFormData() {
    const isAuthenticated = await checkAuthentication();  // Verificar si está autenticado
    if (!isAuthenticated) return;  // Si no está autenticado, no hacer nada

    const userId = localStorage.getItem('user_id'); // Obtener el user_id del almacenamiento local o cualquier otra variable que lo contenga
...
formElements.forEach(element => {
    // Usar 'input' para texto, 'change' para select y textarea para optimización
