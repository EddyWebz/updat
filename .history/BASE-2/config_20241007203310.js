...
historyButton.addEventListener('click', async () => {
    ...
    if (!isAuthenticated) return;  // Si no está autenticado, detener ejecución

    // Obtener la fecha de hoy y la de hace 24 horas
    const today = new Date();
    ...
    // Mostrar fechas en consola para verificar
    console.log('Start Date:', startDate);
    console.log('End Date:', endDate);

    // Ejecutar búsqueda sin query, solo por fechas (últimas 24 horas)
    executeSearch('', startDate, endDate);
});

// Función que maneja la búsqueda y el refresco de la lista de vehículos
function executeSearch(query = '', startDate, endDate) {
    const { startDate: selectedStartDate, endDate: selectedEndDate } = getSelectedDates();  // Obtener fechas predeterminadas
    startDate = startDate || selectedStartDate;  // Usar fechas predeterminadas si no se pasan explícitamente
    endDate = endDate || selectedEndDate;

    // Mostrar fechas y query en consola para verificar
    console.log('Executing Search with:');
    console.log('Query:', query);
    console.log('Start Date:', startDate);
    console.log('End Date:', endDate);

    // Construir la URL de búsqueda según si hay query o no
    const url = query ? 
        `/api/search?query=${encodeURIComponent(query)}&start=${encodeURIComponent(startDate)}&end=${encodeURIComponent(endDate)}` :
        `/api/search?start=${encodeURIComponent(startDate)}&end=${encodeURIComponent(endDate)}`;

    // Mostrar URL de la búsqueda en consola
