document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        // Si no se seleccionan fechas, establecer el rango predeterminado de las últimas 24 horas
        if (!startDateInput || !endDateInput) {
            const now = new Date();
            endDate = now.toISOString().split('T')[0] + ' 23:59:59'; // Hoy a las 23:59
            startDate = new Date(now.getTime() - 24 * 60 * 60 * 1000).toISOString().split('T')[0] + ' 00:00:00'; // Ayer a las 00:00
            console.log('Usando rango predeterminado de las últimas 24 horas:', { startDate, endDate });
        } else {
            // Si se seleccionan fechas, asignar las horas correspondientes
            startDate = `${startDateInput} 00:00:00`; // El día comienza a las 00:00
            endDate = `${endDateInput} 23:59:59`; // El día termina a las 23:59
        }
...
// Escuchar el evento 'actualizarEventos' del servidor
socket.on('actualizarEventos', (evento) => {
    console.log('Nuevo evento recibido:', evento);
    // Agregar el evento recibido a la tabla de eventos
    agregarEventoATabla(evento);
});

// Función para agregar el evento a la tabla en vivo
function agregarEventoATabla(evento) {
    ...
    let eventoHTML = `
        <div class="informacion">
            <p><strong>${evento.usuario}</strong> ${evento.accion} el vehículo con placa <strong>${evento.vehiculo_placa}</strong>, 
            Propietario: <strong>${evento.propietario}</strong>, Habitación: <strong>${evento.habitacion}</strong></p>
            <p class="fecha">Fecha de acción: ${new Date(evento.fecha).toLocaleString()}</p>
        </div>
        <hr>
    ...
    eventoElement.innerHTML = eventoHTML;
    contenedorEventos.prepend(eventoElement); // Agregar el evento al principio de la tabla
}
