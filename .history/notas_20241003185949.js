...
// Ruta para actualizar el comentario y la fecha de expiración de un vehículo
router.post('/comentario', async (req, res) => {
    const { id, comentario, expiracion } = req.body;

    try {
         // Convertir la fecha al formato 'YYYY-MM-DD HH:MM:SS' para MySQL
         const expiracionFormatoMySQL = new Date(expiracion).toISOString().slice(0, 19).replace('T', ' ');
            // Actualiza el comentario y la fecha de expiración
            await pool.query(
                'UPDATE vehiculos SET comentario = ?, comentario_expiracion = ? WHERE id = ?', 
                [comentario, expiracionFormatoMySQL, id]
        );
        res.status(200).send({ message: 'Comentario y expiración actualizados correctamente' });
    } catch (error) {
