...
router.get('/history', verifyJWT, async (req, res) => {
    ...
    try {
        ...
        const history = results.map(vehicle => {
            // Asegúrate de que 'vehicle.datetime' es un formato válido para convertir
            if (vehicle.datetime) {
                // Convertir de UTC a America/Guayaquil
                const fechaLocal = moment.utc(vehicle.datetime).tz('America/Guayaquil').format('YYYY-MM-DD HH:mm:ss');
                console.log('Fecha convertida:', fechaLocal);  // Verifica la fecha en la consola
            }

            return {
                datetime: fechaLocal || vehicle.datetime,  // Usar fecha convertida o la original si hay un problema
                brand: vehicle.brand,
                model: vehicle.model,
                clave: vehicle.clave,
                plate: vehicle.plate,
        ...
        console.log('Historial enviado al frontend:', history);  // Verificar el historial antes de enviarlo
