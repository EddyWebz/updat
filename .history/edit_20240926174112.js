...
router.put('/vehicle/:id', [verifyJWT, upload.array('image', 4)], async (req, res) => {
    ...
    try {
        ...
        if (currentVehicle.brand !== brand) updateDetails += `Marca: de ${currentVehicle.brand} a ${brand}. `;
        if (currentVehicle.model !== model) updateDetails += `Modelo: de ${currentVehicle.model} a ${model}. `;
        if (currentVehicle.clave !== clave) updateDetails += `Clave: de ${currentVehicle.clave} a ${clave}. `;
        if (currentVehicle.plate !== plate) updateDetails += `Placa: de ${currentVehicle.plate} a ${plate}. `;
        if (currentVehicle.color !== color) updateDetails += `Color: de ${currentVehicle.color} a ${color}. `;
        if (currentVehicle.owner !== owner) updateDetails += `Propietario: de ${currentVehicle.owner} a ${owner}. `;
        if (currentVehicle.stayNights !== stayNights) updateDetails += `Noches de estancia: de ${currentVehicle.stayNights} a ${stayNights}. `;
        if (currentVehicle.habitacion !== habitacion) updateDetails += `Habitación: de ${currentVehicle.habitacion} a ${habitacion}. `;
        if (currentVehicle.garage !== garage) updateDetails += `Garaje: de ${currentVehicle.garage} a ${garage}. `;
        if (currentVehicle.observations !== observations) updateDetails += `Observaciones: de ${currentVehicle.observations} a ${observations}. `;
        ...
        // Línea informativa
        const eventDetails = `Actualización de vehículo: Placa: ${plate}, Propietario: ${owner}, Habitación: ${habitacion}. ` +
                             `Cambios realizados: ${updateDetails}`;
                             
        const datetime = new Date();
        await connection.query(eventQuery, [userName, 'Actualización', plate, owner, habitacion, eventDetails, datetime]);
