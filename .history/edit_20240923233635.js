...
router.get('/get-user-role', verifyJWT, async (req, res) => {
    try {
        // Primero, verificar si es un sub-usuario
        const [subUser] = await connection.query('SELECT role FROM sub_usuarios WHERE id = ?', [req.userId]);
        
        if (subUser.length > 0) {
            console.log(`Rol encontrado para el sub-usuario con ID ${req.userId}: ${subUser[0].role}`);
            return res.json({ role: subUser[0].role });
        }
        
        // Si no es sub-usuario, verificar en la tabla usuarios
        const [user] = await connection.query('SELECT role FROM usuarios WHERE id = ?', [req.userId]);
        
        if (user.length > 0) {
            console.log(`Rol encontrado para el usuario con ID ${req.userId}: ${user[0].role}`);
