...
app.post('/api/vehiculos/comentario', async (req, res) => {
    const { id, comentario, expiracion } = req.body;

    try {
        // Actualiza el comentario, la fecha de modificación y la fecha de expiración del comentario
        await db.query(
            'UPDATE vehiculos SET comentario = ?, comentario_expiracion = ? WHERE id = ?', 
            [comentario, expiracion, id]
        );
