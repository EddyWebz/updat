...
const pool = require('./database/database-promise'); // Importa la conexión de la base de datos
...
// Tarea programada para eliminar comentarios a las 2:00 AM todos los días
cron.schedule('19 32 * *', async () => {
    console.log('Ejecutando tarea cron para eliminar comentarios...');

    try {
        // Eliminar comentarios de la base de datos
        const result = await pool.query('UPDATE vehiculos SET comentario = NULL WHERE comentario IS NOT NULL');
        console.log(`Comentarios eliminados: ${result.affectedRows}`);
    } catch (error) {
