...
// Función para convertir HEIC y comprimir imágenes
async function convertAndCompressImage(filePath) {
    try {
        ...
        // Si la imagen es HEIC, usar 'heic-convert' para convertirla a JPEG
        if (metadata.format === 'heic') {
            console.log('Convirtiendo archivo HEIC a JPG...');
            ...
            const outputBuffer = await convert({
                ...
                quality: 0.8,   // opcional, calidad
            });
            fs.writeFileSync(filePath.replace('.heic', '.jpg'), outputBuffer); // Escribir el archivo convertido
            fs.unlinkSync(filePath);  // Eliminar el archivo original HEIC
            filePath = filePath.replace('.heic', '.jpg');
            metadata = await sharp(filePath).metadata();  // Actualizar metadatos para el archivo convertido
        }

        // Si la imagen ya es jpeg o png, o se convirtió a jpg, comprimir la imagen
