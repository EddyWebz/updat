...
router.get('/eventos', verifyJWT, async (req, res) => {
    try {
        ...
        // Lógica para generar el texto adecuado para la actualización
const cambios = [];
if (currentVehicle.stayNights !== stayNights) {
    cambios.push(`Noches de estancia: de ${currentVehicle.stayNights} a ${stayNights}`);
}
if (currentVehicle.plate !== plate) {
    cambios.push(`Placa: de ${currentVehicle.plate} a ${plate}`);
}
// (Añadir más condiciones según los campos actualizables)

const eventDetails = `Actualización del vehículo con placa: ${plate}. Cambios: ${cambios.join(', ')}`;
