...
router.put('/vehicle/:id', [verifyJWT, upload.array('image', 4)], async (req, res) => {
    ...
    try {
        ...
        if (currentVehicle.brand !== brand) updateDetails += `Marca: de ${currentVehicle.brand} se actualizo a ${brand}. `;
        if (currentVehicle.model !== model) updateDetails += `Modelo: de ${currentVehicle.model} se actualizo a ${model}. `;
        if (currentVehicle.clave !== clave) updateDetails += `Clave: de ${currentVehicle.clave} se actualizo  a ${clave}. `;
        if (currentVehicle.plate !== plate) updateDetails += `Placa: de ${currentVehicle.plate} se actualizo a ${plate}. `;
        if (currentVehicle.color !== color) updateDetails += `Color: de ${currentVehicle.color} se actualizo a ${color}. `;
        if (currentVehicle.owner !== owner) updateDetails += `Propietario: de ${currentVehicle.owner} se actualizo a Propietario ${owner}. `;
        if (currentVehicle.stayNights !== stayNights) updateDetails += `Noches de estancia: de ${currentVehicle.stayNights} se actualizo a ${stayNights}. `;
        if (currentVehicle.habitacion !== habitacion) updateDetails += `Habitación: de ${currentVehicle.habitacion} se actualizo a ${habitacion}. `;
        if (currentVehicle.garage !== garage) updateDetails += `Garaje: de ${currentVehicle.garage} se actualizo a Garaje ${garage}. `;
