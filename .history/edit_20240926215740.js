...
router.put('/vehicle/:id', [verifyJWT, upload.array('image', 4)], async (req, res) => {
    ...
    try {
        ...
        if (currentVehicle.color !== color) updateDetails += `Color de ${currentVehicle.color} se actualizo a Color ${color}. `;
        ...
        if (currentVehicle.habitacion !== habitacion) updateDetails += `De habitación ${currentVehicle.habitacion} se actualizo a habitacion ${habitacion}. `;
        if (currentVehicle.garage !== garage) updateDetails += `De garaje ${currentVehicle.garage} se actualizo a garaje ${garage}. `;
