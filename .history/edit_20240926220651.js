...
router.put('/vehicle/:id', [verifyJWT, upload.array('image', 4)], async (req, res) => {
    ...
    try {
        ...
        if (currentVehicle.clave !== clave) updateDetails += `Clave ${currentVehicle.clave} se actualiza a ${clave}. `;
        ...
        if (currentVehicle.owner !== owner) updateDetails += `Propietario ${currentVehicle.owner} se actualiza a Propietario ${owner}. `;
        if (currentVehicle.stayNights !== stayNights) updateDetails += `Noches de estancia de ${currentVehicle.stayNights} noches se actualiza a ${stayNights} noches. `;
