...
router.put('/vehicle/:id', upload.array('image', 4), async (req, res) => {
    ...
    if (req.files && req.files.length > 0) {
        ...
        // Mantener las imágenes existentes si no se suben nuevas
        const currentVehicle = await connection.query('SELECT images FROM vehiculos WHERE id = ?', [vehicleId]);
        imagesJson = currentVehicle[0].images;  // Recuperar las imágenes existentes
    }
