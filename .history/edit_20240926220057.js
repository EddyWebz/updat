...
router.put('/vehicle/:id', [verifyJWT, upload.array('image', 4)], async (req, res) => {
    ...
    try {
        ...
        if (currentVehicle.brand !== brand) updateDetails += `Marca ${currentVehicle.brand} se actualizo a ${brand}. `;
        if (currentVehicle.model !== model) updateDetails += `Modelo ${currentVehicle.model} se actualizo a ${model}. `;
        if (currentVehicle.clave !== clave) updateDetails += `Clave ${currentVehicle.clave} se actualizo  a ${clave}. `;
        if (currentVehicle.plate !== plate) updateDetails += `Placa ${currentVehicle.plate} se actualiza a Placa ${plate}. `;
        if (currentVehicle.color !== color) updateDetails += `Color ${currentVehicle.color} se actualizo a Color ${color}. `;
        if (currentVehicle.owner !== owner) updateDetails += `Propietario ${currentVehicle.owner} se actualizo a Propietario ${owner}. `;
        if (currentVehicle.stayNights !== stayNights) updateDetails += `Noches de estancia ${currentVehicle.stayNights} se actualizo a ${stayNights}.noches `;
